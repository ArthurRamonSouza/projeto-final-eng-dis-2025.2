...
export function setup() {
  ...
  for (let i = 0; i < AD_COUNT; i++) {
    ...
    try {
      ...
      console.warn(`[setup] Resposta inválida ao criar anúncio ${i + 1}.`);
      continue;
    }
  ...
  if (adIds.length === 0) {
    ...
    console.log(`[setup] ${adIds.length} anúncio(s) criado(s) com sucesso.`);
  }
...
export default function (data) {
  const { adIds } = data;
  const adId =
    adIds.length > 0 ? adIds[Math.floor(Math.random() * adIds.length)] : null;
  ...
  if (Math.random() < 0.1) {
    ...
    check(res, {
      "POST /ads → 201 ou 429": (r) => r.status === 201 || r.status === 429,
    });
    ...
    if (res.status === 429) {
      check(res, {
        "POST /ads → 429 possui Retry-After ou mensagem": (r) => {
          try {
            return r.headers["Retry-After"] !== undefined || r.body.length > 0;
          } catch {
            return false;
          }
        },
  ...
  if (adId) {
    ...
    check(res, {
      ...
      "GET /ads/:adId/challenge → possui campo 'challenge' (200)": (r) => {
